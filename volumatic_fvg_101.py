...
def _scan_symbol(
    ...
    if TOUCH_LOOKBACK > 0:
        ...
        if active_now:
            highs_all = df_hi2["high"].to_numpy(float)
            lows_all = df_hi2["low"].to_numpy(float)
            last_idx = len(df_hi2) - 1
            touched_fvgs = []
            for f in active_now:
                key = (sym, f.fvg_id)
                if key in touched:
                ...
                start_idx = max(0, int(f.created_at_bar))
                if start_idx >= last_idx:
                    continue
                prior_highs = highs_all[start_idx:last_idx]
                prior_lows = lows_all[start_idx:last_idx]
                prior_touch = np.any((prior_lows <= top) & (prior_highs >= bottom))
                current_touch = (lows_all[last_idx] <= top) and (highs_all[last_idx] >= bottom)
                if current_touch and not prior_touch:
                    touched_fvgs.append(f)
            if touched_fvgs:
                events.append(
                    {
                        "symbol": sym,
                        ...
                        "fvgs": [
                            ...
                            for f in touched_fvgs
                        ],
                    }
                )
                for f in touched_fvgs:
                    touched_now.add((sym, f.fvg_id))
...
def scan_once(state: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    state_ex = state.get("exchange")
    cached_symbols = state.get("symbols")

    ex = state_ex or _make_exchange()
    if SYMBOLS is not None:
        ...
        try:
            symbols = list_usdtm_symbols(ex)
        except Exception as exc:
            msg = str(exc)
            if "DDoSProtection" in msg or "Too Many Requests" in msg or "418" in msg:
                print(f"[WARN] rate limited loading symbols: {exc}")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [
            executor.submit(_scan_symbol, sym, touched, high_sec, lower_tf, need_1m)
            for sym in symbols
        ]
        for future in as_completed(futures):
            try:
                _, events, new_touched = future.result()
            except Exception as exc:
                msg = str(exc)
                if "DDoSProtection" in msg or "Too Many Requests" in msg or "418" in msg:
                    print(f"[WARN] rate limited: {exc}")
                    time.sleep(RETRY_SLEEP_SECONDS)
                else:
                    print(f"[WARN] scan worker failed: {exc}")
                continue
            for payload in events:
                emit(payload)
            touched.update(new_touched)
