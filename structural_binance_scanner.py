...
AR_GENERIC = {
    LeadershipType.POSITION_LED_CLEAN.value: "قيادة مراكز نظيفة",
    LeadershipType.POSITION_LED_WEAK.value: "قيادة مراكز ضعيفة",
    LeadershipType.ACCOUNT_LED_EARLY.value: "قيادة حسابات مبكرة",
    LeadershipType.ACCOUNT_LED_CROWDED.value: "قيادة حسابات مزدحمة",
    LeadershipType.CONSENSUS_HEALTHY.value: "توافق صحي",
    LeadershipType.CONSENSUS_CROWDED.value: "توافق مزدحم",
    LeadershipType.MIXED.value: "بنية مختلطة",
...
RELIABILITY_RANK = {"High": 3, "Medium": 2, "Low": 1, "Unreliable": 0}
...
def evaluate_oi_state(oi_hist: Sequence[OIHistPoint]) -> Tuple[str, bool, str]:
    if len(oi_hist) < MIN_REQUIRED_POINTS:
        return "Flat / Contradictory", False, "oi history insufficient"
    oi_vals = [p.sum_open_interest for p in oi_hist if not math.isnan(p.sum_open_interest)]
    oi_value_vals = [p.sum_open_interest_value for p in oi_hist if not math.isnan(p.sum_open_interest_value)]
    if len(oi_vals) < MIN_REQUIRED_POINTS:
        return "Flat / Contradictory", False, "oi history insufficient"

    split = len(oi_vals) // 2
    first, second = oi_vals[:split], oi_vals[split:]
    first_slope = slope_like(first)
    second_slope = slope_like(second)
    consistency = sum(1 for i in range(1, len(second)) if second[i] >= second[i - 1]) / max(len(second) - 1, 1)

    if second_slope > first_slope and consistency >= 0.66:
        state = "Gradual Expansion"
    elif second_slope > 0 and consistency < 0.66:
        state = "Late Expansion"
    else:
        peak_idx = max(range(len(oi_vals)), key=lambda i: oi_vals[i])
        state = "Isolated Spike" if 0 < peak_idx < len(oi_vals) - 1 and peak_idx > split else "Flat / Contradictory"

    oi_value_conf = slope_like(oi_value_vals[split:]) >= slope_like(oi_value_vals[:split]) if len(oi_value_vals) >= len(oi_vals) else False
    detail = (
        f"oi_state={state}, first_half_slope={first_slope:.4f}, second_half_slope={second_slope:.4f}, "
        f"consistency={consistency:.2f}, oi_value_conf={oi_value_conf}"
    )
    return state, oi_value_conf, detail
...
def evaluate_flow_state(futures_candles: Sequence[Candle], spot_candles: Sequence[Candle]) -> Tuple[FlowState, List[str]]:
    ...
    if cvd_support is None:
        evidence.append("proxy_spot_cvd unavailable")
    else:
        evidence.append(f"proxy_spot_cvd {'supports' if cvd_support else 'diverges'}")

    baseline = statistics.median([compute_taker_buy_pressure(futures_candles[max(0, i - 6): i + 1]) for i in range(12, len(futures_candles)) if not math.isnan(compute_taker_buy_pressure(futures_candles[max(0, i - 6): i + 1]))]) if len(futures_candles) > 18 else all_bp
    ign_relative = ign_bp > baseline if not math.isnan(ign_bp) and not math.isnan(baseline) else False
    sustain_relative = sus_bp >= ign_bp if not math.isnan(sus_bp) and not math.isnan(ign_bp) else False

    if ign_relative and sustain_relative and (cvd_support is None or cvd_support):
        return FlowState.SUPPORTIVE_SUSTAINED, evidence
    if ign_relative and not sustain_relative:
        return FlowState.IGNITION_ONLY, evidence
    if ign_relative and cvd_support is False:
...
def detect_acceptance(
    ...
    oi_hist: Optional[Sequence[OIHistPoint]] = None,
    flow_state: Optional[FlowState] = None,
    funding: Optional[Sequence[float]] = None,
    ...
    breakout_idx = next((i for i, c in enumerate(action) if c.close > key_level), None)
    ...
    retest_hold = any((c.low <= key_level and c.close >= key_level) for c in post[2:8])
    continuation = post[-1].close > key_level and post[-1].close >= max(c.close for c in post[: max(len(post) - 2, 1)])
    oi_not_collapsing = True
    if oi_hist and len(oi_hist) >= 10:
        oi_vals = [p.sum_open_interest for p in oi_hist if not math.isnan(p.sum_open_interest)]
        if len(oi_vals) >= 10:
            oi_not_collapsing = statistics.mean(oi_vals[-5:]) >= statistics.mean(oi_vals[-10:-5])
    funding_crowded = False
    if funding:
        funding_crowded, _ = is_funding_crowded(funding)
    flow_persistent = flow_state in {FlowState.SUPPORTIVE_SUSTAINED, None}
    ...
    ev = [
        ...
        f"oi_not_collapsing={oi_not_collapsing}",
        f"flow_persistent={flow_persistent}",
        f"funding_crowded={funding_crowded}",
    ]

    if close_above and not fast_reject and (retest_hold or continuation) and oi_not_collapsing and flow_persistent and not funding_crowded:
...
def detect_preparation_structure(data: SymbolTimeframeData) -> PreparationStructure:
    ...
    oi_state, oi_value_conf, oi_detail = evaluate_oi_state(data.oi_hist)
    ...
    accounts_crowding_ahead = not math.isnan(acc_tr) and acc_tr > 0 and (math.isnan(pos_tr) or acc_tr > pos_tr)
    ...
    return PreparationStructure(
        status=status,
        oi_gradual_expand=oi_state == "Gradual Expansion",
...
def detect_squeeze_phase(data: SymbolTimeframeData) -> SqueezeAssessment:
    ...
    close_rets = [pct_change(closes[i - 1], closes[i]) for i in range(1, len(closes))]
    oi_rets = [pct_change(oi[i - 1], oi[i]) for i in range(1, len(oi))]
    price_vol = statistics.pstdev([x for x in close_rets if not math.isnan(x)]) if len(close_rets) > 6 else 0.0
    oi_vol = statistics.pstdev([x for x in oi_rets if not math.isnan(x)]) if len(oi_rets) > 6 else 0.0
    price_ignition = p_late > statistics.median([x for x in close_rets if not math.isnan(x)]) + price_vol
    oi_compression = oi_late < statistics.median([x for x in oi_rets if not math.isnan(x)]) - oi_vol

    if price_ignition and oi_compression:
        rebuild = (oi_early >= oi_late) or (not math.isnan(pos_tr) and pos_tr > 0)
    ...
    crowded_explosion = price_ignition and oi_late > statistics.median([x for x in oi_rets if not math.isnan(x)]) and funding_crowded and (math.isnan(pos_tr) or pos_tr <= 0) and acc_tr > 0
...
def determine_leadership(data: SymbolTimeframeData, prep: PreparationStructure) -> LeadershipType:
    ...
    acc_tr = ratio_trend(data.top_account_ratio)
    glob_tr = ratio_trend(data.global_account_ratio)
    oi_state, oi_value_conf, _ = evaluate_oi_state(data.oi_hist)

    if oi_state == "Gradual Expansion" and oi_value_conf and pos_tr > 0 and (math.isnan(acc_tr) or pos_tr > acc_tr):
        return LeadershipType.POSITION_LED_CLEAN
    if oi_state in {"Gradual Expansion", "Late Expansion"} and pos_tr > 0:
        return LeadershipType.POSITION_LED_WEAK
    if oi_state in {"Gradual Expansion", "Late Expansion"} and acc_tr > 0 and (math.isnan(pos_tr) or pos_tr <= 0):
        return LeadershipType.ACCOUNT_LED_EARLY if not prep.accounts_crowding_ahead else LeadershipType.ACCOUNT_LED_CROWDED
    if oi_state == "Gradual Expansion" and oi_value_conf and pos_tr > 0 and acc_tr > 0 and glob_tr > 0:
        return LeadershipType.CONSENSUS_HEALTHY if not prep.accounts_crowding_ahead else LeadershipType.CONSENSUS_CROWDED
    return LeadershipType.MIXED
...
def merge_multi_tf(
    ...
    acceptance_assess = detect_acceptance(acceptance.futures_candles, acceptance.oi_hist, flow_state, acceptance.funding)
...
def classify_structure(
    ...
    leadership_supportive = leadership in {
        LeadershipType.POSITION_LED_CLEAN,
        LeadershipType.POSITION_LED_WEAK,
        LeadershipType.CONSENSUS_HEALTHY,
        LeadershipType.ACCOUNT_LED_EARLY,
    }
    flow_supportive = flow_state == FlowState.SUPPORTIVE_SUSTAINED
    ...
    if acceptance.state == BreakoutAcceptanceState.ACCEPTED:
        if leadership == LeadershipType.CONSENSUS_HEALTHY and prep_strong and flow_supportive:
            return FinalClassification.CONSENSUS
        if leadership in {LeadershipType.POSITION_LED_CLEAN, LeadershipType.POSITION_LED_WEAK} and prep_strong and flow_supportive:
            return FinalClassification.POSITION_LED
        if leadership == LeadershipType.ACCOUNT_LED_EARLY and prep_main.status == "qualified" and flow_supportive:
            return FinalClassification.ACCEPTED_BREAKOUT
        if flow_state in {FlowState.IGNITION_ONLY, FlowState.WEAK, FlowState.DIVERGENT}:
            return FinalClassification.MIXED
    ...
    if prep_main.status == "qualified":
        if leadership in {LeadershipType.POSITION_LED_CLEAN, LeadershipType.POSITION_LED_WEAK}:
            return FinalClassification.GENUINE_EARLY_BULLISH
        if leadership == LeadershipType.CONSENSUS_HEALTHY:
            return FinalClassification.CONSENSUS
...
def compute_confidence(
    ...
    checks.append(leadership in {LeadershipType.POSITION_LED_CLEAN, LeadershipType.POSITION_LED_WEAK, LeadershipType.CONSENSUS_HEALTHY, LeadershipType.ACCOUNT_LED_EARLY})
    checks.append(flow_state == FlowState.SUPPORTIVE_SUSTAINED)
    checks.append(acceptance.state == BreakoutAcceptanceState.ACCEPTED)
    checks.append(squeeze.state in {SqueezeState.REAL_SHORT_SQUEEZE, SqueezeState.NONE})
    score = sum(checks)
    if score >= 5:
        return "High"
    if score >= 4:
...
def bucket_for_result(result: StructuralResult) -> DisplayBucket:
    ...
    leadership_ok = result.leadership_type in {
        LeadershipType.POSITION_LED_CLEAN.value,
        LeadershipType.POSITION_LED_WEAK.value,
        LeadershipType.CONSENSUS_HEALTHY.value,
        LeadershipType.ACCOUNT_LED_EARLY.value,
    }
    golden_rule_match = (
...
def print_result_ar(result: StructuralResult, color: str) -> None:
    ...
    print(
        ...
        f"الموثوقية: {result.confidence_score}{ANSI_RESET}"
    )
...
def safe_fetch_symbol_tf(client: BinanceClient, symbol: str, timeframe: str, lookback: int) -> Optional[SymbolTimeframeData]:
    try:
        ...
        print(f"{ANSI_RED}فشل جلب البيانات: symbol={symbol} timeframe={timeframe} error={exc}{ANSI_RESET}")
        return None
...
def analyze_single(symbol: str, data: SymbolTimeframeData) -> StructuralResult:
    ...
    flow_state, flow_ev = evaluate_flow_state(data.futures_candles, data.spot_candles)
    acceptance = detect_acceptance(data.futures_candles, data.oi_hist, flow_state, data.funding)
...
def run_scanner() -> None:
    ...
    ranked.sort(
        key=lambda x: (
            ...
            -RELIABILITY_RANK.get(x[1].confidence_score, 0),
            x[1].symbol,
        )
    )
