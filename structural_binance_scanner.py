...
def print_result(result: StructuralResult) -> None:
    if OUTPUT_MODE == "json":
        print(asdict(result))
        return

    print(
        f"{result.symbol} | class={result.final_structural_classification} | "
        f"leadership={result.leadership_type} | flow={result.flow_state} | "
        f"acceptance={result.breakout_acceptance_state} | squeeze={result.squeeze_state} | "
...
def run_scanner() -> None:
    ...
    results.sort(key=lambda r: (r.final_structural_classification, -r.confidence_score, r.symbol))
    for r in results:
        print_result(r)
