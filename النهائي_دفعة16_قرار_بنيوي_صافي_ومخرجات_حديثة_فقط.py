...
DYNAMIC_SETTINGS: Dict[str, Any] = {
    ...
    "APPLY_CANDIDATE_CAP_AFTER_FILTER": True,
...
def analyze_symbol_structural(
    ...
    """
    try:
        features = build_features(client, symbol, symbol_meta, ticker_24h, mark_info)
        ...
        # 1) build_raw_materials
        raw_materials = build_raw_materials(symbol, features)
        ...
        # 2) build_features (already built above) + 3) build_memory_context
        result = build_memory_context(symbol, result)

        # 4) build_structural_inference + 5) build_hypotheses
        prev_state = load_previous_symbol_state(symbol)
        result = build_structural_inference(symbol, result)

        # 6) build_execution_verdict
        result = run_execution_verdict_stage(symbol, result, prev_state, pre_center_baseline)

        # 7) render_output
        result = render_output(result)

        # 8) persist_memory
        result = persist_memory(symbol, result)
        return result
...
# ============================================================
# مراحل الأوركستريتور (تنظيم الدفعة 2)
# ============================================================
...
def build_features(
    ...
    return build_reference_market_features(client, symbol, symbol_meta, ticker_24h, mark_info)


def build_raw_materials(symbol: str, features: Dict[str, Any]) -> Dict[str, Any]:
...
def build_memory_context(symbol: str, result: Dict[str, Any]) -> Dict[str, Any]:
    result = dict(result)
    features = safe_dict_from_api(result.get("market_features"))
...
def build_hypotheses(features: Dict[str, Any], result: Dict[str, Any], leader_info: Dict[str, Any]) -> Dict[str, Any]:
    result = dict(result)
...
def build_structural_inference(symbol: str, result: Dict[str, Any]) -> Dict[str, Any]:
    result = dict(result)
    features = safe_dict_from_api(result.get("market_features"))
    ...
    result = build_hypotheses(features, result, leader_info)
...
def run_execution_verdict_stage(symbol: str, result: Dict[str, Any], prev_state: Dict[str, Any], pre_center_baseline: Dict[str, Any]) -> Dict[str, Any]:
    result = dict(result)
    features = safe_dict_from_api(result.get("market_features"))
    ...
    # تحسب مرة واحدة فقط بعد اكتمال الحكم النهائي
    ...
    final_case_match = match_closest_case(build_case_vector(features, result), load_case_library())
    ...
    result["case_similarity_context"] = build_asset_case_similarity_context(asset_local_memory_latest, features, result)
...
def render_output(result: Dict[str, Any]) -> Dict[str, Any]:
    result = dict(result)
    # مرة واحدة فقط في النهاية بعد اكتمال الحكم
...
def persist_memory(symbol: str, result: Dict[str, Any]) -> Dict[str, Any]:
...
def print_symbol_result(result: Dict[str, Any]) -> None:
    ...
    print(f"- نظام السوق V2: {view.get('market_regime_v2_ar', 'مرحلة انتقالية')}")
    ...
    print(f"- نية المشاركين: {view.get('actor_intent_ar', 'نية غير محسومة')}")
    print(f"- نافذة الدخول: {view.get('entry_window_state_ar', 'تجنب/انتظار')}")
    ...
    print("\nتسلسل البحث")
    print(f"- السياق: {decision_trace.get('context', view.get('regime_pattern_ar', 'غير معروف'))}")
    print(f"- بيئة السوق: {decision_trace.get('regime_v2', view.get('market_regime_v2_ar', 'مرحلة انتقالية'))}")
    print(f"- القيادة: {decision_trace.get('leadership', view.get('leader_type_ar', 'غير معروف'))}")
    print(f"- نية الطرف الغالب: {decision_trace.get('actor_intent', view.get('actor_intent_ar', 'نية غير محسومة'))}")
    print(f"- علاقة OI: {decision_trace.get('oi_relation', view.get('oi_state_ar', 'غير معروف'))}")
    print(f"- التدفق: {decision_trace.get('flow', _safe_state_text(result.get('flow_quality'), 'غير محسوم'))}")
    print(f"- الزناد: {decision_trace.get('trigger', view.get('trigger_pattern_ar', 'غير معروف'))}")
    print(f"- القبول: {decision_trace.get('acceptance', _safe_state_text(result.get('acceptance_reason'), 'غير محسوم'))}")
    print(f"- الفرضية: {decision_trace.get('hypothesis', view.get('primary_hypothesis_ar', 'غير محسومة'))}")
    print(f"- القرار: {decision_trace.get('decision', state_text)}")
    print(f"- نافذة التنفيذ: {decision_trace.get('entry_window', view.get('entry_window_state_ar', 'تجنب/انتظار'))}")
    print(f"- الإبطال: {decision_trace.get('invalidation', view.get('invalidation_reason_ar', 'لا يوجد إبطال محدد بعد'))}")
