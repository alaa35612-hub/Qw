...
# ============================================================
# Analysis Mode (Research-driven structural engine)
# ============================================================
# single: run all structural phases on PRIMARY_TIMEFRAME only
# multi: Preparation(1h)->Ignition(15m)->Acceptance/Continuation(5m)
ANALYSIS_MODE = "multi"   # "single" or "multi"
PRIMARY_TIMEFRAME = "5m"  # used when ANALYSIS_MODE="single"
...
def _resolve_phase_timeframes(features: Dict[str, Any]) -> Dict[str, str]:
    tf_pool = safe_dict_from_api(features.get("multi_tf"))
    if _safe_state_text(ANALYSIS_MODE, "multi") == "single":
        tf = PRIMARY_TIMEFRAME if PRIMARY_TIMEFRAME in tf_pool else "5m"
        return {"prep_tf": tf, "ignition_tf": tf, "accept_tf": tf}
    return {"prep_tf": "1h", "ignition_tf": "15m", "accept_tf": "5m"}


def detect_preparation_structure(features: Dict[str, Any], phase_tfs: Dict[str, str]) -> Dict[str, Any]:
    prep_tf = _safe_state_text(phase_tfs.get("prep_tf"), "1h")
    tf_payload = safe_dict_from_api(safe_dict_from_api(features.get("multi_tf")).get(prep_tf))
    ratio_payload = safe_dict_from_api(safe_dict_from_api(features.get("ratio_multi_tf")).get(prep_tf))
    gap = safe_dict_from_api(ratio_payload.get("gap"))
    pos = safe_dict_from_api(ratio_payload.get("position"))
    acc = safe_dict_from_api(ratio_payload.get("account"))
    oi_delta = safe_float(tf_payload.get("oi_delta_3"), 0.0)
    ret_3 = safe_float(tf_payload.get("ret_3"), 0.0)
    pos_last = safe_float(pos.get("last"), 0.0)
    acc_last = safe_float(acc.get("last"), 0.0)
    position_lead = safe_float(gap.get("position_lead"), 0.0)
    account_lead = safe_float(gap.get("account_lead"), 0.0)

    if position_lead > 0.02 and oi_delta > 0:
        leadership = "Position-Led Build-Up"
    elif account_lead > 0.02 and oi_delta > 0:
        leadership = "Account-Led Expansion"
    elif pos_last > 1.0 and acc_last > 1.0 and oi_delta > 0:
        leadership = "Consensus Expansion"
    else:
        leadership = "Contradictory Structure"

    funding = safe_float(features.get("funding_current"), 0.0)
    crowding = "neutral"
    if funding > 0.0015:
        crowding = "crowded_long_risk"
    elif funding < -0.0015:
        crowding = "crowded_short_risk"

    prep_state = "prepared" if oi_delta > 0 and abs(ret_3) <= 1.8 else ("already_expanding" if oi_delta > 0 else "not_prepared")
    return {
        "structure_phase": "Preparation",
        "timeframe": prep_tf,
        "preparation_state": prep_state,
        "leadership_type": leadership,
        "oi_state": "expanding_exposure" if oi_delta > 0 else "flat_or_contracting",
        "crowding_context": crowding,
    }


def detect_squeeze_phase(features: Dict[str, Any], phase_tfs: Dict[str, str], prep: Dict[str, Any]) -> Dict[str, Any]:
    ign_tf = _safe_state_text(phase_tfs.get("ignition_tf"), "15m")
    tf_payload = safe_dict_from_api(safe_dict_from_api(features.get("multi_tf")).get(ign_tf))
    ret_1 = safe_float(features.get("ret_1"), 0.0)
    ret_3 = safe_float(tf_payload.get("ret_3"), 0.0)
    oi_delta_1 = safe_float(features.get("oi_delta_1"), 0.0)
    oi_delta_3 = safe_float(tf_payload.get("oi_delta_3"), 0.0)
    flow_spike = safe_float(features.get("recent_buy_ratio"), 0.5) >= 0.56 and safe_float(features.get("trade_expansion_last"), 0.0) >= 1.15
    pre_vulnerability = prep.get("crowding_context") == "crowded_short_risk" and prep.get("leadership_type") in {"Position-Led Build-Up", "Account-Led Expansion"}
    ignition = ret_1 > 0 and oi_delta_1 > 0 and flow_spike
    post_expansion = ret_3 > 0 and oi_delta_3 > 0
    if pre_vulnerability and ignition and post_expansion:
        squeeze_state = "Real Short Squeeze"
    elif ignition and not post_expansion:
        squeeze_state = "Short Covering Only"
    else:
        squeeze_state = "No Squeeze"
    return {"structure_phase": "Ignition", "timeframe": ign_tf, "squeeze_state": squeeze_state, "ignition_flow": flow_spike}


def detect_acceptance(features: Dict[str, Any], phase_tfs: Dict[str, str]) -> Dict[str, Any]:
    accept_tf = _safe_state_text(phase_tfs.get("accept_tf"), "5m")
    tf_payload = safe_dict_from_api(safe_dict_from_api(features.get("multi_tf")).get(accept_tf))
    close_above = bool(features.get("close_above_breakout", False))
    hold = bool(features.get("hold_above_breakout_3bars", False))
    ret_3 = safe_float(tf_payload.get("ret_3"), 0.0)
    oi_delta_1 = safe_float(features.get("oi_delta_1"), 0.0)
    flow_ratio = safe_float(features.get("recent_buy_ratio"), 0.5)
    flow_state = "aggressive_persistent" if flow_ratio >= 0.55 and oi_delta_1 >= 0 else "ignition_only_or_weak"
    if close_above and hold and ret_3 > 0:
        acceptance_state = "Accepted Breakout"
    elif close_above and not hold:
        acceptance_state = "Breakout Not Yet Accepted"
    elif features.get("breakout_touched", False):
        acceptance_state = "Ignition without acceptance"
    else:
        acceptance_state = "No breakout context"
    return {"structure_phase": "Acceptance", "timeframe": accept_tf, "breakout_acceptance_state": acceptance_state, "flow_state": flow_state}


def classify_structure(prep: Dict[str, Any], squeeze: Dict[str, Any], acceptance: Dict[str, Any]) -> Dict[str, Any]:
    leadership = _safe_state_text(prep.get("leadership_type"), "Contradictory Structure")
    squeeze_state = _safe_state_text(squeeze.get("squeeze_state"), "No Squeeze")
    acceptance_state = _safe_state_text(acceptance.get("breakout_acceptance_state"), "No breakout context")
    flow_state = _safe_state_text(acceptance.get("flow_state"), "ignition_only_or_weak")
    if acceptance_state == "Accepted Breakout" and leadership == "Position-Led Build-Up":
        final, conf = "Position-Led Expansion", 0.80
    elif acceptance_state == "Accepted Breakout" and leadership == "Account-Led Expansion":
        final, conf = "Account-Led Expansion", 0.74
    elif squeeze_state == "Real Short Squeeze" and acceptance_state in {"Accepted Breakout", "Breakout Not Yet Accepted"}:
        final, conf = "Real Short Squeeze", 0.78
    elif squeeze_state == "Short Covering Only":
        final, conf = "Short Covering Only", 0.58
    elif acceptance_state == "Ignition without acceptance" and flow_state == "aggressive_persistent":
        final, conf = "Flow Spike Failure", 0.56
    elif acceptance_state == "Breakout Not Yet Accepted":
        final, conf = "Genuine Early Bullish Structure", 0.62
    elif acceptance_state == "No breakout context" and leadership == "Contradictory Structure":
        final, conf = "Fake Breakout", 0.52
    else:
        final, conf = "Genuine Early Bullish Structure", 0.55
    return {"final_structural_classification": final, "confidence_score": round(conf, 2)}


def analyze_symbol_structural_engine(
    client: BinanceFuturesPublicClient,
    symbol: str,
    symbol_meta: Dict[str, Any],
    ticker_24h: Dict[str, Any],
    ...
    try:
        features = build_reference_market_features(client, symbol, ticker_24h, mark_info)
        phase_tfs = _resolve_phase_timeframes(features)
        prep = detect_preparation_structure(features, phase_tfs)
        squeeze = detect_squeeze_phase(features, phase_tfs, prep)
        acceptance = detect_acceptance(features, phase_tfs)
        classified = classify_structure(prep, squeeze, acceptance)
        final_class = classified["final_structural_classification"]
        actionable = final_class in {"Position-Led Expansion", "Account-Led Expansion", "Real Short Squeeze"}
        return {
            "symbol": symbol,
            "timeframe_mode": _safe_state_text(ANALYSIS_MODE, "multi"),
            "structure_phase": "Continuation" if acceptance.get("breakout_acceptance_state") == "Accepted Breakout" else "Acceptance",
            "leadership_type": prep.get("leadership_type"),
            "oi_state": prep.get("oi_state"),
            "flow_state": acceptance.get("flow_state"),
            "breakout_acceptance_state": acceptance.get("breakout_acceptance_state"),
            "squeeze_state": squeeze.get("squeeze_state"),
            "final_structural_classification": final_class,
            "confidence_score": classified.get("confidence_score", 0.5),
            "market_features": features,
            "acceptance_state": "yes" if acceptance.get("breakout_acceptance_state") == "Accepted Breakout" else "no",
            "execution_state": "قابلة للتنفيذ الآن" if actionable else "مرصودة لكنها غير قابلة للتنفيذ بعد",
            "structural_execution_state": "actionable_now" if actionable else "discovered_not_actionable",
            "final_bucket": "Actionable now" if actionable else "Discovered but not actionable",
            "actionable_now": actionable,
            "decision_reason": f"{prep.get('leadership_type')} | {acceptance.get('breakout_acceptance_state')} | {squeeze.get('squeeze_state')}",
            "analysis_center": "structural_market_intelligence_engine",
        }
    except Exception as exc:
        return {
            "symbol": symbol,
            "error": str(exc),
            "final_structural_classification": "Flow Spike Failure",
            "timeframe_mode": _safe_state_text(ANALYSIS_MODE, "multi"),
            "structure_phase": "Preparation",
            "leadership_type": "Contradictory Structure",
            "oi_state": "unknown",
            "flow_state": "unknown",
            "breakout_acceptance_state": "No breakout context",
            "squeeze_state": "No Squeeze",
            "confidence_score": 0.0,
            "execution_state": "فاشلة",
            "structural_execution_state": "failed",
            "final_bucket": "Failed",
            "actionable_now": False,
            "decision_reason": "Internal structural engine error",
            "analysis_center": "structural_market_intelligence_engine",
        }
...
analyze_symbol = analyze_symbol_structural_engine
...
def prepare_candidates(
    ...
    for symbol, symbol_meta in symbol_universe.items():
        ...
        # Structural engine mandate: no market filtering logic.
        if symbol_meta.get("status") != "TRADING" or symbol_meta.get("quoteAsset") != "USDT" or symbol_meta.get("contractType") != "PERPETUAL":
            skipped_counter["non_active_or_non_usdt_perpetual"] += 1
            continue
