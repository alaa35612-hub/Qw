...
class SmartMoneyAlgoProE5:
    ...
    def _destroy_golden_zone(self) -> None:
        ...
        if not isinstance(zone, Box):
            ...
            self.bxty = 0
            return
        self._golden_zone_meta.pop(id(zone), None)
        if zone in self.boxes:
            self.boxes.remove(zone)
        self.bxf = None
        self.bxty = 0

    def _invalidate_golden_zone_if_broken(self) -> None:
        zone = getattr(self, "bxf", None)
        if not isinstance(zone, Box) or self.bxty == 0:
            return
        lower, upper = self._zone_bounds(zone)
        if math.isnan(lower) or math.isnan(upper):
            return
        ...
        if math.isnan(high) or math.isnan(low):
            return
        broken = (self.bxty == 1 and low < lower) or (self.bxty == -1 and high > upper)
        if broken:
            self._destroy_golden_zone()
    ...
    def _update_bar(self) -> None:
        ...
        if self.inputs.structure_util.isOTE:
            ...
            if oi1 is not None:
                ...
                if not math.isnan(candidate):
                    oi1_value = candidate
            prev_oi1_value: Optional[float] = None
            if isinstance(self.prev_oi1, float) and not math.isnan(self.prev_oi1):
                prev_oi1_value = self.prev_oi1
            should_create_zone = False
            if oi1_value is not None:
                if self.bxty == 0:
                    should_create_zone = (
                        prev_oi1_value is None
                        or not math.isclose(oi1_value, prev_oi1_value, rel_tol=1e-9, abs_tol=1e-9)
                    )
                else:
                    should_create_zone = True
            if should_create_zone:
                self._destroy_golden_zone()
                top_val = (
                    ot
                    if (ot is not None and not math.isnan(ot))
                    else self.series.get("high")
                )
                bot_val = (
                    ob
                    if (ob is not None and not math.isnan(ob))
                    else self.series.get("low")
                )
                left = int(oi1_value)
                self.bxf = self.box_new(left, time_val, top_val, bot_val, self.inputs.structure_util.oteclr)
        ...
        self._invalidate_golden_zone_if_broken()
