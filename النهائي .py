        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
            ...
            for future in concurrent.futures.as_completed(future_map):
                ...
                try:
                    ...
                    _maybe_print(f"تعذر جلب شموع {symbol}: {exc}", flush=True)
...
def _fetch_recent_ohlcv(
    ...
    try:
        ...
        _maybe_print(f"تعذر جلب شموع {symbol} على إطار {timeframe}: {exc}", flush=True)
        return []
...
def _binance_pick_symbols(
    ...
    if explicit:
        ...
        try:
            ...
            _maybe_print(f"فشل تحميل الأسواق للتحقق من الرموز المحددة يدويًا: {exc}")
        ...
        if invalid:
            ...
            _maybe_print(f"تحذير: سيتم تجاهل الرموز غير الصحيحة: {', '.join(invalid_sorted)}")
        return BinanceSymbolSelection(valid, [], False, False)

    try:
        markets = exchange.load_markets()
    except Exception as exc:
        _maybe_print(f"فشل تحميل أسواق Binance: {exc}")
        return BinanceSymbolSelection([], [], False, False)
    ...
    if not usdtm_markets:
        _maybe_print("لم يتم العثور على عقود Binance USDT-M نشطة.")
        return BinanceSymbolSelection([], [], False, False)

    try:
        tickers = exchange.fetch_tickers()
    except Exception as exc:
        _maybe_print(f"تعذر جلب بيانات التيكر، سيتم استخدام فرز افتراضي: {exc}")
        tickers = {}
...
def print_symbol_summary(index: int, symbol: str, timeframe: str, candle_count: int, metrics: Dict[str, Any]) -> None:
    ...
    print(
        f"  {ANSI_LABEL}{'تم تعطيل عرض الإشارات الحية':<26}{ANSI_RESET}: "
        f"{ANSI_VALUE_ZERO}—{ANSI_RESET}",
        flush=True,
    )
...
def _latest_golden_zone_box(runtime: "SmartMoneyAlgoProE5") -> Optional[Box]:
    for box in reversed(getattr(runtime, "boxes", [])):
        if isinstance(box, Box) and box.text.strip() == "Golden zone":
            return box
    return None
...
def scan_binance(
    ...
    *,
    ...
    for idx, symbol in enumerate(all_symbols):
        ...
        golden_box = _latest_golden_zone_box(runtime)
        ...
        if (
            ...
            low = min(golden_box.bottom, golden_box.top)
            high = max(golden_box.bottom, golden_box.top)
            price_value = float(current_price)
            if low <= price_value <= high:
                golden_zone_hits.append(
                    {
                        "symbol": symbol,
                        "low": low,
                        "high": high,
    ...
    return primary_runtime, golden_zone_hits


def _print_golden_zone_hits(hits: Sequence[Dict[str, Any]]) -> None:
    print("Symbols in Golden Zone:")
    for entry in hits:
        symbol = str(entry.get("symbol", "")).upper()
        low = format_price(entry.get("low"))
        high = format_price(entry.get("high"))
        price = format_price(entry.get("price"))
        print(f"{symbol} | {low} – {high} | {price}")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        "--timeframe",
        type=str,
        default=EDITOR_AUTORUN_DEFAULTS.timeframe,
    ...
    parser.add_argument(
        "--lookback",
        type=int,
        default=EDITOR_AUTORUN_DEFAULTS.candle_limit,
    ...
    parser.add_argument(
        "--max-symbols",
        type=int,
        default=EDITOR_AUTORUN_DEFAULTS.max_symbols,
        help="أقصى عدد من الرموز لمسحها في دورة واحدة",
    )
    parser.add_argument(
        "--continuous",
        "--continuous-scan",
        ...
        default=AUTORUN_CONTINUOUS_SCAN,
    ...
    if args.max_symbols is not None and args.max_symbols < 0:
        parser.error("--max-symbols يجب أن يكون رقمًا غير سالب")

    def log(stage: str) -> None:
        _maybe_print(stage, flush=True)
    ...
    if args.pullback_report:
        ...
        _maybe_print(f"Pullback report written to {args.outfile}")
        log("Coverage")
        tracer.emit()
        return 0
    ...
    try:
        while True:
            ...
            with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
                runtime, golden_hits = scan_binance(
                    args.timeframe,
                    args.lookback,
                    manual_symbols,
                    args.concurrency,
                    ...
                    symbol_selector=DEFAULT_BINANCE_SYMBOL_SELECTOR,
                )
            _print_golden_zone_hits(golden_hits)
            tracer.emit()
        ...
        _maybe_print("تم إيقاف المسح من قبل المستخدم.", flush=True)
    return 0
