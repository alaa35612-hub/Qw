...
# Local cache for OHLCV
CACHE_DIR = "data_cache"

# Rising symbols prefilter (scan only rising symbols first)
RISE_FILTER_ENABLED = True
RISE_FILTER_ONLY = True
RISE_FILTER_TFS = ["1d", "4h", "1h", "15m", "5m", "1m"]
RISE_FILTER_LOOKBACK = {
    "1d": 1,
    "4h": 3,
    "1h": 6,
    "15m": 8,
...
def _cache_path(symbol: str, timeframe: str) -> str:
    ...
    return os.path.join(CACHE_DIR, f"{safe}__{timeframe}.csv")

def load_cached_ohlcv(symbol: str, timeframe: str) -> pd.DataFrame:
...
def save_cached_ohlcv(symbol: str, timeframe: str, df: pd.DataFrame) -> None:
    os.makedirs(CACHE_DIR, exist_ok=True)
    path = _cache_path(symbol, timeframe)
    df = df.sort_values("time").drop_duplicates("time")
    df.to_csv(path, index=False)

def fetch_ohlcv_cached(ex, symbol: str, timeframe: str, limit: int, max_bars: int) -> pd.DataFrame:
    cached = load_cached_ohlcv(symbol, timeframe)
    since_ms = None
    if not cached.empty:
        since_ms = int(cached["time"].iloc[-1].value // 10**6) + 1
    fresh = fetch_ohlcv_paged(ex, symbol, timeframe, since_ms=since_ms, limit=limit, max_bars=max_bars)
    if cached.empty:
        merged = fresh
    else:
    ...
    save_cached_ohlcv(symbol, timeframe, merged)
    return merged
...
def _rise_filter_pass(ex, sym: str) -> Optional[float]:
    """
    Returns max % change across configured rise filter timeframes.
    Returns None if data is insufficient or no timeframe passes.
    """
    best = None
    for tf in RISE_FILTER_TFS:
        lookback = int(RISE_FILTER_LOOKBACK.get(tf, 1))
        min_pct = float(RISE_FILTER_MIN_PCT.get(tf, 0.0))
        candles = ex.fetch_ohlcv(sym, timeframe=tf, limit=lookback + 1)
        ...
        if past == 0:
            continue
        pct = ((last - past) / past) * 100.0
        if pct >= min_pct:
            if best is None or pct > best:
                best = pct
    return best

def select_rising_symbols(ex, symbols: List[str]) -> List[str]:
    rising = []
    for sym in symbols:
        try:
            pct = _rise_filter_pass(ex, sym)
            if pct is not None:
                rising.append((sym, pct))
        except Exception:
            continue
    rising.sort(key=lambda x: x[1], reverse=True)
    return [s for s, _ in rising]
...
def scan_once(state: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if RISE_FILTER_ENABLED:
        rising = select_rising_symbols(base_ex, symbols)
        if RISE_FILTER_ONLY:
            symbols = rising
        else:
            tail = [s for s in symbols if s not in set(rising)]
            symbols = rising + tail
    ...
    def process_symbol(sym: str):
        ...
        try:
            ex = _make_exchange()
            # fetch chart tf candles (cached)
            df_high = fetch_ohlcv_cached(ex, sym, TIMEFRAME, limit=1500, max_bars=num_candles)
            if not df_high or len(df_high) < 150:
                status = "skip_short"
                return
            if not isinstance(df_high, pd.DataFrame):
            ...
            df_1m = fetch_ohlcv_cached(ex, sym, "1m", limit=1500, max_bars=need_1m)
            if df_1m.empty:
                status = "skip_no_1m"
                return
