...
def _worker_scan_symbol(symbol: str, config: Dict[str, object], fetch_limit: int) -> Tuple[List[Signal], List[ZoneEvent]]:
    ...
    ltf_fetch_bars = max(5, int(config.get("event_age_bars", 0)) + 2)
    ohlcv_ltf = fetch_ohlcv_safe(ex, symbol, ltf, ltf_fetch_bars)
    if not ohlcv_ltf:
        return [], []
    ...
    unique_events = _filter_by_age_ts(unique_events, ts_ltf, int(config.get("event_age_bars", 0)))
