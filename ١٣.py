...
LAST_REFERENCE_BUCKET = None


def _compute_reference_bucket(now_dt=None):
    """
    توحيد لقطة القرار على حدود 5m لمنع تبدل الطباعة داخل نفس نافذة Binance الأساسية.
    """
    if now_dt is None:
        now_dt = datetime.utcnow()
    bucket_minute = (now_dt.minute // 5) * 5
    bucket_dt = now_dt.replace(minute=bucket_minute, second=0, microsecond=0)
...
def quick_filter():
    """
    إزالة الفلترة المركبة: تمرير الكون كاملاً إلى طبقة العائلات الأربع فقط.
    """
    print("🔍 تحميل الكون الكامل بدون فلترة مسبقة...")
    symbols = [s for s in (TRADING_SYMBOLS_SET or set()) if s and not is_symbol_invalid(s)]
    if not symbols:
        symbols = get_all_usdt_perpetuals()
    symbols = sorted(set(symbols))
    if MAX_SYMBOLS_PER_CYCLE and len(symbols) > MAX_SYMBOLS_PER_CYCLE:
        symbols = symbols[:MAX_SYMBOLS_PER_CYCLE]
    print(f"✅ {len(symbols)} عملة جاهزة للتحليل المرجعي المباشر")
    return symbols
...
def update_signal_stability(signal_data):
    ...
    oi_state = signal_data.get('oi_state', 'NEUTRAL')
    now = time.time()
    current_snapshot = signal_data.get('cycle_snapshot_ts') or CYCLE_SNAPSHOT_TS
    with SIGNAL_STABILITY_LOCK:
        prev = SIGNAL_STABILITY.get(symbol)
        stable_family = bool(prev and prev.get('family') == family)
        stable_oi = bool(prev and prev.get('oi_state') == oi_state)
        prev_snapshot = (prev or {}).get('snapshot_ts')
        same_bucket = bool(prev_snapshot and current_snapshot and prev_snapshot == current_snapshot)
        stable_window = max(240, SCAN_INTERVAL_MINUTES * 180)
        if prev and stable_family and stable_oi and same_bucket:
            cycles = int(prev.get('cycles', 1) or 1)
        elif prev and stable_family and stable_oi and (now - prev.get('ts', 0)) <= stable_window:
            cycles = int(prev.get('cycles', 1) or 1) + 1
        else:
            cycles = 1
...
def main():
    ...
    print("ℹ️ تم تعطيل تدريب HMM نهائيًا حسب وضع التشغيل المرجعي الصارم.")

    learning_system = LearningSystem(db)

    while True:
        start_time = time.time()
        global CYCLE_SNAPSHOT_TS, LAST_REFERENCE_BUCKET
        CYCLE_SNAPSHOT_TS = _compute_reference_bucket(datetime.utcnow())
        bucket_changed = CYCLE_SNAPSHOT_TS != LAST_REFERENCE_BUCKET
        if bucket_changed:
            with CYCLE_CACHE_LOCK:
                CYCLE_DYNAMIC_CACHE.clear()
                CYCLE_CONTEXT_CACHE.clear()
            LAST_REFERENCE_BUCKET = CYCLE_SNAPSHOT_TS
            print(f"🧭 تم فتح لقطة مرجعية جديدة 5m: {CYCLE_SNAPSHOT_TS}")
        else:
            print(f"🧭 إعادة استخدام لقطة 5m الحالية: {CYCLE_SNAPSHOT_TS}")
